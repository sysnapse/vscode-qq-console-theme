    ...
    for (let v of message) {
        switch (v.type) {
            ...
            case "reply":
                ...
                msg += `<a href="#${v.data.id}" onclick="document.querySelector('#${filterMsgIdSelector(v.data.id).replace(/\\/g, "\\\\")}')?.nextElementSibling.animate([{'background':'#ffff99'}],{duration: 3000})">[回复]</a>`;
                break;
            case "rps":
                msg += "[猜拳]";
                break;
